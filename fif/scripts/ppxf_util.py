...
def determine_goodpixels_muse(logLam, lamRangeTemp, z, AO=True):
    """
    Generates a list of goodpixels to mask a given set of gas emission
    lines. This is meant to be used as input for PPXF.
...
#                     -----[OII]-----    Hdelta   Hgamma   Hbeta   -----[OIII]-----   [OI]    -----[NII]-----   Halpha   -----[SII]-----
    ...
    flag |= np.exp(logLam) > 6500   # Mask edges of
    flag |= np.exp(logLam) < 4700   # MUSE
    
    if AO:
        flag |= (np.exp(logLam) > 5750.00) & (np.exp(logLam) < 6020.00)
